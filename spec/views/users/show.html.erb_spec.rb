...
RSpec.describe 'User show page:', type: :feature do
  ...
  scenario 'I can see the users first 3 posts' do
    visit user_path(@user_one.id)
    expect(page).to have_content(@user_one.recentthreeposts.first.title)
    expect(page).to have_content(@user_one.recentthreeposts.second.title)
    expect(page).to have_content(@user_one.recentthreeposts.third.title)
  end
  ...
  scenario 'When I click a users post, it redirects me to that posts show page' do
    visit user_path(@user_one.id)
    click_link("Post ##{@user_one.recentthreeposts.first.id}")
    expect(page).to have_current_path(user_post_path(@user_one.id, @user_one.recentthreeposts.first.id))
  end
